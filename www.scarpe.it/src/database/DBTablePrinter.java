...
public class DBTablePrinter {
	...
	private static class Column {
		/**
		 * Constructs a new <code>Column</code> with a column label, generic SQL
		 * type and type name (as defined in <a target="_blank" href=
		 * "http://docs.oracle.com/javase/8/docs/api/java/sql/Types.html"> java.
		 * sql.Types </a>)
		 *
		 * @param label
		...
		/**
		 * Adds a <code>String</code> representation (<code>value</code>) of a
		 * value to this column object's {@link #values} list. These values will
		 * come from each row of a <a target="_blank" href=
		 * "http://docs.oracle.com/javase/8/docs/api/java/sql/ResultSet.html">
		 * ResultSet </a> of a database query.
		 *
		 * @param value
		...
		/**
		 * Returns the column value at row index <code>i</code>. Note that the
		 * index starts at 0 so that <code>getValue(0)</code> will get the value
		 * for this column from the first row of a <a target="_blank" href=
		 * "http://docs.oracle.com/javase/8/docs/api/java/sql/ResultSet.html">
		 * ResultSet</a>.
		 *
		 * @param i
		...
		/**
		 * Returns the value of the {@link #justifyFlag}. The column values will
		 * be printed using <code>String.format</code> and this flag will be
		 * used to right or left justify the text.
		 *
		...
		/**
		 * Sets {@link #justifyFlag} to <code>"-"</code> so that the column
		 * value will be left justified when printed with
		 * <code>String.format</code>. Typically numbers will be right justified
		 * and text will be left justified.
		 */
		 ...
		 * string (<code>""</code>) to justify right, dash (<code>-</code>) to
		 * justify left.
		 *
		 * @see #justifyLeft()
		 */
		...
		/**
		 * Column type category. The columns will be categorised according to
		 * their column types and specific needs to print them correctly.
		 */
	 ...
	 * {@link #DEFAULT_MAX_TEXT_COL_WIDTH} will be used to limit the width of
	 * text columns (like a <code>VARCHAR</code> column).
	 *
	 * @param conn
	 ...
	 * Overloaded method to print rows of a <a target="_blank" href=
	 * "http://docs.oracle.com/javase/8/docs/api/java/sql/ResultSet.html">
	 * ResultSet</a> to standard out using <code>maxStringColWidth</code> to
	...
	public static void printResultSet(ResultSet rs, int maxStringColWidth) {
		try {
			 ...
			 * created all Column objects, iterated over the ResultSet rows,
			 * populated the column values and adjusted the column widths. We
			 * cannot start printing just yet because we have to prepare a row
			 * separator String.
			 */
			...
			for (Column c : columns) {
				...
				int paddingSize = diff / 2; // InteliJ says casting to int is
											// redundant.

				// Cool String repeater code thanks to user102008 at
				// stackoverflow.com
				// (http://tinyurl.com/7x9qtyg) "Simple way to repeat a string
				// in java"
	...
	/**
	 * Takes a generic SQL type and returns the category this type belongs to.
	 * Types are categorized according to print formatting needs:
	 * <p>
	 * Integers should not be truncated so column widths should be adjusted
	 * without a column width limit. Text columns should be left justified and
	 * can be truncated to a max. column width etc...
	 * </p>
	 *
	 * See also: <a target="_blank" href=
	 * "http://docs.oracle.com/javase/8/docs/api/java/sql/Types.html"> java.sql.
	 * Types</a>
	 *
	 * @param type
	 *            Generic SQL type
	...
	/**
	 * Default maximum width for text columns (like a <code>VARCHAR</code>)
	 * column.
	 */
	private static final int DEFAULT_MAX_TEXT_COL_WIDTH = 150;

	/**
	 * Column type category for <code>CHAR</code>, <code>VARCHAR</code> and
	 * similar text columns.
	 */
